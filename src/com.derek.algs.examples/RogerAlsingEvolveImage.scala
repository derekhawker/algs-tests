...
import com.derek.algs.structures.concrete.infinite.neighbourhood.{INTraitSeqRef, AlsingPolygon}
...
object RogerAlsingEvolveImage {
  ...
  def main(args: Array[String]) {
    ...
    val positionPopulation =
      generateRandomPopulation(numPopulation, numPolygons, goalImageWidth, 0, goalImageHeight, 0,
        255, 0)

    val velocityPopulation =
      generateRandomPopulation(numPopulation, numPolygons, goalImageWidth / 20.0,
        -goalImageWidth / 20.0, goalImageHeight / 20.0, -goalImageHeight / 20.0,
        25, 0)
  ...
  private def updatePosition(position: Polygon,
    ...
    def updateVertexPosition(i: Int,
                             n: Int): Double = {
      val newpos = (((posVerts(i).productElement(n).asInstanceOf[Double] + velVerts(i).productElement(n)
  ...
  private def generateRandomPopulation(numPopulation: Int,
                                       numPolygons: Int,
                                       lbXVertex: Double,
                                       ubXVertex: Double,
                                       lbYVertex: Double,
